...
const StepFirst = ({ formData, handelChangeInput, handelNextStep }: any) => {
  const initialStepInformation = [
    {
      label: "Emploi",
      value: formData.isEmploi,
      name: "isEmploi",
      description: "Information par rapport à l'emploi",
    },
    {
      label: "Education",
      value: formData.isEducation,
      name: "isEducation",
      description: "Information par rapport à l'éducation",
    },
    {
      label: "Logement",
      value: formData.isLogement,
      name: "isLogement",
      description: "Information par rapport au logement",
    },
    {
      label: "Transport",
      value: formData.isTransport,
      name: "isTransport",
      description: "Information par rapport au transport",
    },
  ];
  ...
  return (
    <div>
      <h1 className="mt-2 text-xl font-bold text-blue-900">
        Step 1: Les domaines de témoignage
      </h1>
      <p className="text-sm font-normal text-gray-900">
        Selectionner au moins un domaine
      </p>
      <ul className="grid w-full my-4 gap-6 md:grid-cols-3">
        {initialStepInformation.map((item) => (
          <li key={item.name}>
            <input
              type="checkbox"
              id={`react-option-${item.name}`}
              name={item.name}
            ...
            />
            <label
              htmlFor={`react-option-${item.name}`}
      ...
      <div className="lg:my-4 sm:my-2 flex justify-end items-center">
        {formData.isEducation ||
        formData.isEmploi ||
        formData.isLogement ||
